...
import { Student, GameState, GameConfig } from './types';
...
export default function App() {
    ...
    if (s1.isZombie || s2.isZombie) {
      setStudents(students.map(s => {
        if (s.id === id1 || s.id === id2) {
          const updated = { ...s, touchedThisRound: true, points: s.points + 1 };
          if (!s.isZombie) {
  ...
  const handleCure = (id: string) => {
    if (currentRound === 3) return;
    
    setStudents(students.map(s => {
      if (s.id === id) {
        if (s.infectedThisRound) {
  ...
  const nextRound = () => {
    // Inactivity check: Students who didn't touch become zombies
  ...
  const StartView = () => (
    ...
    >
      ...
      <Button size="lg" onClick={() => setView('SETUP_CONFIG')} variant="neon">
      ...
      >
        <Card>
          ...
          <div className="mt-12 flex justify-end">
            <Button onClick={() => {
              setConfig({ roundTime: localRoundTime, totalRounds: 3 });
      ...
      >
        <Card>
            ...
            <AnimatePresence initial={false}>
              {students.map((s) => (
                ...
                >
                  ...
                  <button onClick={() => removeStudent(s.id)} className="text-zinc-500 hover:text-red-500 transition-colors flex-shrink-0">
                    <X className="w-4 h-4" />
                  </button>
                </motion.div>
              ))}
          ...
          <div className="mt-12 flex justify-between items-center">
            ...
            <Button onClick={() => setView('SETUP_ZOMBIES')} disabled={students.length < 2}>
              다음 단계 <ChevronRight className="w-5 h-5" />
            </Button>
          </div>
        </Card>
      ...
      >
        <Card>
          ...
          <div className="mt-12 flex justify-between items-center">
            ...
            <Button onClick={startRound} disabled={zombieCount === 0} variant="neon">
      ...
      <div className="max-w-6xl mx-auto py-8 px-4">
            ...
            {currentRound < 3 && (
              <Button 
                ...
                onClick={() => {
                  ...
                  if (student?.infectedThisRound) {
                    handleCure(student.id);
                    setSelectedIds([]);
                  }
                }}
        ...
        <div className="fixed bottom-8 right-8 z-40">
          <Button 
            variant="ghost" 
            size="sm"
            onClick={() => setShowTeacherPage(true)}
            ...
            >
              <div className="max-w-4xl mx-auto">
                <div className="flex justify-between items-center mb-8">
                  <h2 className="text-3xl font-black text-purple-500">교사 전용 현황판</h2>
                  <Button variant="ghost" onClick={() => setShowTeacherPage(false)}>
                    <X className="w-8 h-8" />
                  </Button>
                </div>
  ...
  const ResultsView = () => {
    ...
    const winner = actualZombies > actualHumans ? 'ZOMBIE' : actualHumans > actualZombies ? 'HUMAN' : 'DRAW';
      ...
      >
        ...
        {!isAnimating && (
          <motion.div
            ...
            animate={{ scale: 1, opacity: 1 }}
          >
            <h1 className={`text-8xl font-black mb-8 tracking-tighter ${winner === 'ZOMBIE' ? 'text-green-500' : winner === 'HUMAN' ? 'text-purple-500' : 'text-white'}`}>
              {winner === 'DRAW' ? 'DRAW' : `${winner} WIN`}
            </h1>

            <div className="max-w-2xl mx-auto mb-12">
              <h3 className="text-xl font-bold mb-4 text-zinc-400">개별 승점 순위</h3>
    ...
    <div className="min-h-screen bg-black text-white font-sans selection:bg-purple-500/30">
      ...
      <main className="relative z-10 container mx-auto px-4 py-8">
        <AnimatePresence mode="wait">
          {view === 'START' && <StartView key="start" />}
          {view === 'SETUP_CONFIG' && <SetupConfigView key="setup-config" />}
          {view === 'SETUP_STUDENTS' && <SetupStudentsView key="setup-students" />}
          {view === 'SETUP_ZOMBIES' && <SetupZombiesView key="setup-zombies" />}
